...
static void BuildMatrix(ConversionMatrixA* matrix, double Kr, double Kb, int shift, int full_scale, int bits_per_pixel)
{
  int Sy, Suv, Oy;
...
inline void col2yuv(uint32_t* c, uint8_t* y, uint8_t* u, uint8_t* v, ConversionMatrixA* m)
{
...
void make_sub_img16(ASS_Image* img, uint8_t** sub_img0, uint32_t width, int bits_per_pixel, int rgb, ConversionMatrixA *mx)
{
    ...
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j += 2) {
            if (srcA0[j] + srcA1[j]) {
                ...
                dstU[k]  = blend2A(srcA0[j], srcU0[j],
                                  srcA1[j], srcU1[j], dstU[k]);
                dstV[k]  = blend2A(srcA0[j], srcV0[j],
    ...
    for (i = 0; i < height; i += 2) {
        for (j = 0; j < width; j += 2) {
            ...
            if (srcA00[j] + srcA01[j] + srcA10[j] + srcA11[j]) {
                ...
                dstU[k]   = blend4A(srcA00[j], srcU00[j],
                                   srcA01[j], srcU01[j],
                                   srcA10[j], srcU10[j],
                                   srcA11[j], srcU11[j], dstU[k]);
                dstV[k]   = blend4A(srcA00[j], srcV00[j],
                                   srcA01[j], srcV01[j],
                                   srcA10[j], srcV10[j],
  ...
  for (unsigned int i = 0; i < height; i++) {
    for (unsigned int j = 0; j < width / 4; j++) {
      ...
      if (srcA[jy] + srcA[jy + 1] + srcA[jy + 2] + srcA[jy + 3]) {
        ...
        dstU[j] = blend4A(srcA[jy + 0], srcU[jy + 0],
          srcA[jy + 1], srcU[jy + 1],
          srcA[jy + 2], srcU[jy + 2],
          srcA[jy + 3], srcU[jy + 3],
          dstU[j]);
        dstV[j] = blend4A(srcA[jy + 0], srcV[jy + 0],
          srcA[jy + 1], srcV[jy + 1],
          srcA[jy + 2], srcV[jy + 2],
          srcA[jy + 3], srcV[jy + 3],
          dstV[j]);
...
PVideoFrame AVSC_CC assrender::GetFrame(int n, IScriptEnvironment* env)
{
    //const Function *f;
    udata* ud; //= (udata*)f->user_data;
    ASS_Image* img;
    PVideoFrame src;
    ...
    if (!ud->isvfr) {
        ...
        ts = (int64_t)n * (int64_t)1000 * (int64_t)vi.fps_denominator / (int64_t)vi.fps_numerator;
    } else {
        ts = ud->timestamp[n];
    }
    ...
    if (img) {
        ...
        if (vi.IsPlanarRGB() && !ud->greyscale) {
          if (vi.IsRGB()) {
            // planar RGB as 444
            data[0] = src->GetWritePtr(PLANAR_R);
            data[1] = src->GetWritePtr(PLANAR_G);
            data[2] = src->GetWritePtr(PLANAR_B);
            pitch[0] = src->GetPitch();
          }
          else {
            data[0] = src->GetWritePtr(PLANAR_Y);
            data[1] = src->GetWritePtr(PLANAR_U);
            data[2] = src->GetWritePtr(PLANAR_V);
            pitch[0] = src->GetPitch(PLANAR_Y);
            pitch[1] = src->GetPitch(PLANAR_U);
          }
        }
        else {
          data[0] = src->GetWritePtr();
          pitch[0] = src->GetPitch();
        }
